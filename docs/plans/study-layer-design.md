...
### 3.1 Study는 그 자체로 독립 학습 세션입니다

2026-08-06 정정입니다. 초기 구현은 Study에 자체 `outcomes`를 두지 않고
`outcome_coverage`로 Unit 학습성과만 가리켰습니다. 그 결과 자료를 학습하려면
반드시 Unit을 거쳐야 했고, 매핑이 잘 되는 Unit을 먼저 고른 뒤 그 Unit의
학습성과로 교재를 만드는 역전이 실제로 발생했습니다. 자료의 주된 내용이 아니라
Unit에 붙이기 쉬운 곁가지가 학습 주제가 되었습니다.

**자료는 그 자료가 소개하는 내용 자체를 학습하기 위한 세션입니다.** 따라서
Study는 다음을 가집니다.

- `outcomes`: 이 자료를 학습했을 때 무엇을 할 수 있게 되는지. Unit과 독립이며
  대응하는 Unit이 없어도 성립합니다. ID는 `outcome.study.<...>` 공간을 씁니다.
- `resource_refs`: 이 자료를 학습하기 위한 교재. Resource의 `owner.kind`에
  `study`를 허용하여 Unit을 거치지 않고 자기 자료를 소유합니다.

`outcome_coverage`는 **선택적 교차연결**입니다. 대응하는 Unit이 있을 때 나중에
그 Unit을 학습하며 "이 자료로 이미 본 범위"를 찾기 위한 것이지, 학습의 경로가
아닙니다. Unit이 없다고 학습이 막히지 않습니다.

```text
Study ── outcomes + 자기 교재 ─────────► 독립 학습 (주 경로)
   └──── outcome_coverage ────────────► Unit 교차연결 (있을 때만)
```

자료 한 편으로 도달 가능한 범위는 대개 D0에서 D1입니다. D2 이상은 새로운
입력에서의 독립 수행 증거가 필요하므로 자료 학습만으로 주장하지 않습니다.
...
**`outcome_coverage`는 자료가 무엇을 다루는지에 대한 판정이며 학습자가 그
학습성과를 달성했다는 뜻이 아닙니다.** 이 둘을 섞으면 자료를 읽은 것만으로
역량을 갖췄다고 기록하게 됩니다. 학습자의 수행 증거는 Unit의 `validation`과
전이과제에서 별도로 확인하며, Study는 어떤 자료가 무엇을 다루는지만 남깁니다.
`full`은 자료가 그 학습성과의 범위를 모두 다룬다는 뜻이지 학습자가 통과했다는
뜻이 아닙니다.
